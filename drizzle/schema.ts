...
export const scheduledJobs = mysqlTable(
  "scheduled_jobs",
  {
    ...
    callbackToken: varchar("callbackToken", { length: 64 }),
    activeCallbackToken: varchar("activeCallbackToken", { length: 64 }),
    completedCallbackToken: varchar("completedCallbackToken", { length: 64 }),
    createdAt: timestamp("createdAt").defaultNow().notNull(),
    updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
  },
  table => ({
